...
int main_rpc_refiner(int c, char *v[])
{
    ...
    if (c != 8) {
        fprintf(stderr, "usage:\n\t"
                "%s rpc.xml tie_points.txt step_deriv step_grad nb_iter bool_direct out.txt"
              // 0     1      2               3           4       5          6         7
                "\n", *v);
        fprintf(stderr,"c = %d\n",c);
        return EXIT_FAILURE;
    ...
    {   
        ...
        gradient_descent(address, step_deriv, step_grad, true,
    ...
    else
    {
        ...
        gradient_descent(addressi, step_deriv, step_grad, false,
            &rpc_coef, list_tie_points, nb_tie_points, nb_iter);
