...
if __name__ == "__main__":
    ...
    if len(sys.argv) not in [7,8]:
        print("Usage: {} im_in rpc_in im_out rpc_out filter scale_x [scale_y]".format(sys.argv[0]))
        print("Use scale_x (resp. scale_y) > 1 to zoom in, and scale_x (resp. scale_y) < 1 to zoom out")
        print("Filter should be one of the following gdal filters: {}".format(available_filters))
        sys.exit(1)
